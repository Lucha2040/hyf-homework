...
function getShorties(arr) {
  return arr["title"].length < 5;
}
const shortMovies = movies.filter(getShorties);
console.log(shortMovies);

//2) Get the movies with a long title
function getLongies(arr) {
  return arr["title"].length > 50;
}
const longMovies = movies.filter(getLongies);
console.log(longMovies);

//3)Get movies made between 1980-1989
function get80s(arr) {
  return arr["year"] <= 1989 && arr["year"] >= 1980;
}

const loveThe80s = movies.filter(get80s);
...
function addTag(arr) {
  if (arr.rating >= 7) {
    arr.tag = "Good";
  } else if (arr.rating >= 4) {
    return (arr.tag = "Average");
  } else {
    return (arr.tag = "Bad");
  }
}

const moviesWithTag = movies.map(addTag);
console.log(movies[657]["tag"]); //Here I got confused. Because map add the tag to the existing array "movies" and left my new array with all undefined keys except the "tag"
// console.log(moviesWithTag[657]["tag"]);
...
const higherThan6 = movies.filter(getHigh6).map(getRating);
...
function getTitle(arr) {
  return arr["title"].toLowerCase();
}

function getWords(i) {
  if (i.match(/surfer/gi) || i.match(/benjamin/gi) || i.match(/alien/gi)) {
    return i
  }
}

const moviesTitleLC = movies.map(getTitle)
const movies6b = moviesTitleLC.filter(getWords);
console.log(movies6b.length); 
...
//Note: I got stuck in this exercise. I found this solution in https://www.w3resource.com/javascript-exercises/javascript-array-exercise-20.php but it doesn't seem to work. 
